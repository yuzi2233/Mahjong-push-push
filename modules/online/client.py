...
class gobangClient(QWidget):
    ...
    def __init__(self, cfg, nickname, server_ip, parent=None, **kwargs):
        ...
        self.server_ipport = ('45.147.54.174', cfg.PORT)
        self.roomnum=server_ip
        ...
        room=self.roomnum
        self.tcp_socket.sendall(room.encode())
        ...
        while True:
            ...
            if (b-a).seconds>3:
                print("时间到！")
                QMessageBox.information(self, '提示', '无人连接，游戏即将结束！')
                exit()
            if self.tcp_socket.recv(1024):
                print(self.tcp_socket.recv(1024))
                break
