...
class aiGobang():
    ...
    def act(self, history_record):
        ...
        for item in history_record:
            self.aiplayer_list.append((item[0], item[1])) #拿出这个棋子
            if item[0]>self.maxx:
                self.maxx=item[0]
            if item[0]<self.minx:
                self.minx=item[0]
        ...
        print(self.next_point,self.maxx,self.minx,self.maxy,self.miny)
        return self.next_point
    '''负极大值搜索, alpha+beta剪枝'''
    def __doSearch(self, is_ai_round, depth, alpha, beta):
        '''如果收到游戏结束的信号，则返回/如果已经到了最低的一层，则执行评估函数'''
        ...
        for next_step in blank_list:
            ...
            if not self.__hasNeighbor(next_step):
                continue
            '''是否在边框内'''
            if not self.__inRange(next_step):
                continue
            if is_ai_round:
                '''对手回合添加位置'''
            ...
            '''递归寻找，到下一层，Alpha和Beta倒置（相当于下一层是对方的回合，要用对方的角度去评估这一步的分数）'''
            ...
            '''进行剪枝，如果有利于对方，则舍去，如果不利于对方，则可以选择'''
            if value > alpha:
    ...
    '''限制搜索边框'''
    def __inRange(self,next_step):
        if next_step[0]>self.maxx+2 or next_step[0]<self.minx-1 or next_step[1]>self.maxy+2 or next_step[1]<self.miny-2:
            return False
        return True
    '''计算得分'''
